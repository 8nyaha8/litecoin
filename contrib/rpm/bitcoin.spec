...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5887
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5888
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15887
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15890
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 5887
	%{_sbindir}/semanage port -d -p tcp 5888
	%{_sbindir}/semanage port -d -p tcp 15887
	%{_sbindir}/semanage port -d -p tcp 15890
	for selinuxvariant in %{selinux_variants}; do
